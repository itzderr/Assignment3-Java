...
public class BankAccount {
    ...
    public boolean transactionFee(double fee) {
        ...
        for(times = 0; times <= transactionCount; times++) {
            pay += (fee * times);
        }

        BankAccount todo2 = new BankAccount(-50);
        todo2.deposit(10.00);
        todo2.deposit(50.00);
        todo2.deposit(10.00);
        todo2.deposit(70.00);

        if (todo2.balance >= pay) {
            return true;
        }
        return false;
    }
    ...
    public boolean transfer(double amount, BankAccount other) {
        ...
        BankAccount todo3 = new BankAccount();

        if(this.balance > (amount + 5)){
            this.balance = this.balance - amount - 5.00;
            todo3.balance = todo3.balance + amount;
            return true;
        }else if(this.balance > 0 && this.balance < (amount + 5)){
            this.balance = 0;
            todo3.balance = todo3.balance + this.balance - 5;
            return true;
        }
        return false;
    }
    ...
    public String toString() {
        ...
        if (balance >= 0){
            sta = String.format(name + ", $" + df.format(balance));
            return sta;
        }else {
            balance = balance * (-1);
            sta = String.format(name + ", -$" + df.format(balance));
