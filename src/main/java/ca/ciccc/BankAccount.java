...
public class BankAccount {
    ...
    public boolean transfer(double amount, BankAccount other) {
        ...
        if (currBalance > amount + serviceFee) {
            this.withdraw(amount + serviceFee);
            other.deposit(amount);

            return true;
        } else if (currBalance >= serviceFee) {
            this.withdraw(currBalance);
            other.deposit(currBalance - serviceFee);
            return true;
        }
        ...
        public String toString () {
            ...
            String balance = this.getBalance() > 0 ? "" : "-";
            balance += "$" + String.format("%.2f", Math.abs(this.getBalance()));
