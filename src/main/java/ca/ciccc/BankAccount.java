...
public class BankAccount {
    public static final double TRANSFER_FEE = 5.0;
    private String id;
    private String name;
    ...
    public boolean transfer(double amount, BankAccount other) {
        if (amount <= 0 || getBalance() < TRANSFER_FEE) {
            return false;
        }
        withdraw(TRANSFER_FEE);

        double totalAmount = getBalance() < amount ? getBalance() : amount;

        withdraw(totalAmount);
        other.deposit(totalAmount);
    ...
    public String toString() {
        return String.format("%s, %s", getName(), getBalanceFormatted());
    }
